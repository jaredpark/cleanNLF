makeErrData = function(pred, line, actual, delta){
  ...
  if (numBetsWithAction == 0){
    stop('Zero bets with action')
  }
  classErr = 1 - sum(classification == actualClass)/numBetsWithAction
  adjClass = ifelse(line > pred - delta & line < pred + delta, 'noBet', classification)
  ...
  if (adjNumBetsWithAction == 0){
    stop('Zero adjusted bets with action')
  }
...
pickRule = function(predRulesByStats, colName = 'rmse'){
...
addColumn = function(data, vector, newColName){
  ...
  names(toAdd) = newColName
  isNew = !is.element(newColName, names(data))
  if (isNew){
    out = cbind(data, toAdd)
